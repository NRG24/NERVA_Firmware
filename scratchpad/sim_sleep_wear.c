...
static void t_baseline_unchanged(void)
{
	...
	/*
	 * 0, not 3: the minutes that end a session are the wearer getting up,
	 * not restlessness during it, so sleep.c backs them out (see
	 * end_session()). This read 3 before that fix.
	 */
	CHECK(sleep_restless_minutes() == 0, "restless %u, wanted 0",
